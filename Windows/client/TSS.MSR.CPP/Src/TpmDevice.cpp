...
void hexdump(const BYTE* buf, unsigned int size);
...
void TpmTcpDevice::DispatchCommand(const ByteVec& cmdBuf)
{
    ...
    if (mbHexDumpTransactionsEnable)
    {
        cout << "SENT:\n";
        hexdump(cmdBuf.data(), (unsigned int)cmdBuf.size());
    }
...
ByteVec TpmTcpDevice::GetResponse()
{
    ...
    {
        cout << "RECEIVED:\n";
        hexdump(resp.data(), (unsigned int)resp.size());
    }
...
void TpmTbsDevice::DispatchCommand(const ByteVec& outBytes)
{
    ...
    {
        cout << "SENT:\n";
        hexdump(outBytes.data(), (unsigned int)outBytes.size());
        cout << "RECEIVED:\n";
        hexdump(resultBuffer, resSize);
    }
...
#define hexdump_putc(c) putc(c, stdout)
/*----------------------------------------------------------------------------*/
static const char hex1[] = "0123456789abcdef";
static void print_hex(ULONG_PTR val, int bc)
{
    int i;
    ULONG_PTR c;
    for (i = 0; i < bc; i++)
...
void hexdump(const BYTE* buf, unsigned int size)
{
    ...
    print_hex((ULONG_PTR)(ptr-buf), sizeof(ptr));
    hexdump_putc(' ');
    for (i = 0; i < size; i++)
    {
        print_hex(ptr[i], 1);
        ...
        else if (cnt == 16)
        {
            ...
            if (i != (size - 1))
            {
                print_hex((unsigned long)(ULONG_PTR)(ptr - buf + i + 1), sizeof(ptr));
                hexdump_putc(' ');
                cnt = 0;
            }
        }
